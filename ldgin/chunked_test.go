...
func TestChunkedRenderer_Render(t *testing.T) {
	buildResultString := func(strs []string) string {
		return strings.Join(strs, lineEnd)
	}
	...
	convey.Convey(t.Name(), t, func(c convey.C) {
		...
		c.Convey("render succ", func(c convey.C) {
			c.Convey("only reader", func(c convey.C) {
				handler := WrapHandler(func(c *Context) (ChunkedRenderer, Error) {
			...
			c.Convey("other options", func(c convey.C) {
				...
				c.So(w.Header().Get(chunkedHeaderKey), convey.ShouldEqual, "gzip, chunked")
		...
		c.Convey("read fail", func(c convey.C) {
			...
			c.So(w.Body.String(), convey.ShouldEqual, buildResultString([]string{
				"9",
				strings.Repeat("abc", 3),
				fmt.Sprintf("code: %d", lderr.ErrHttpRenderBody.Code()),
				fmt.Sprintf("message: %s", lderr.ErrHttpRenderBody.Error()),
				"details:",
				"    test error",
				"",
			}))
